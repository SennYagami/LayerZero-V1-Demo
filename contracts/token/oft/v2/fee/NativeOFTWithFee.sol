...
contract NativeOFTWithFee is OFTWithFee, ReentrancyGuard {
    ...
    function withdraw(uint _amount) external nonReentrant {
        require(
            ...
            "NativeOFTWithFee: Insufficient balance."
        );
    ...
    function sendFrom(
        ...
        require(
            _amount >= _minAmount,
            "NativeOFTWithFee: amount is less than minAmount"
        );
    }
    ...
    function _debitFromNative(
        ...
        uint _amount,
        uint16 _dstChainId
    ) internal returns (uint messageFee, uint amount) {
        ...
        messageFee = msg.sender == _from
            ? _debitMsgSender(amount, newMsgValue)
            : _debitMsgFrom(_from, amount, newMsgValue);
    }

    function _debitMsgSender(
        uint _amount,
        uint currentMsgValue
    ) internal returns (uint messageFee) {
        uint msgSenderBalance = balanceOf(msg.sender);

        if (msgSenderBalance < _amount) {
            require(
                msgSenderBalance + currentMsgValue >= _amount,
                "NativeOFTWithFee: Insufficient msg.value"
            );
    ...
    function _debitMsgFrom(
        ...
        if (msgFromBalance < _amount) {
            require(
                msgFromBalance + currentMsgValue >= _amount,
                "NativeOFTWithFee: Insufficient msg.value"
            );
